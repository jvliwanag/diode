...
case class ModelProxy[S](modelReader: ModelR[_, S], dispatch: AnyRef => _) {
  ...
  def zoom[T](f: S => T)(implicit feq: FastEq[_ >: T]) =
    ModelProxy(modelReader.zoom(f), dispatch)
...
trait ReactConnector[M <: AnyRef] { circuit: Circuit[M] =>
  ...
  /**
    ...
    */
  def wrap[S <: AnyRef, C](zoomFunc: M => S)(
  ...
  /**
    ...
    * @return The component returned by `compB`
    */
  def wrap[S <: AnyRef, C](modelReader: ModelR[_, S])(
      compB: ModelProxy[S] => ReactElement)(
      implicit feq: FastEq[_ >: S]): ReactElement = {
    implicit object aType extends ActionType[Any]
    compB(
      ModelProxy(modelReader, (action: AnyRef) => circuit.dispatch(action)))
  }
  ...
  /**
    ...
    * @return A React component
    */
  def connect[S >: Null <: AnyRef](modelReader: ModelR[_, S])(
      compB: ModelProxy[S] => ReactElement)(
      implicit feq: FastEq[_ >: S]): ReactElement = {
